...
function AnnonceSuivis() {
    ...
    function UpdatePicture(idPhoto) {
        axios({
            method: "POST",
            url: "http://codx.fr:8080/UpdatePhotoForFollow/" + idPhoto,
            data: {
                urlphoto: selectedPhotoUri,
            }
        }).then((resp) => {
            if (resp.data === 'OK') {
            }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.panel}>
                <Text style={styles.reservation}> N° {AnnonceReserve.numero}</Text>
                                    ...
                                    {new Date(item.datePoste).toDateString() === currentDate.toDateString() ? (
                                        <View>
                                            <View style={styles.buttonContainer}>
                                                ...
                                                <TouchableOpacity style={styles.button} onPress={UpdatePicture(item.idPhoto)}>
                                                    <Text style={styles.buttonText}>Enregistrer</Text>
                                                </TouchableOpacity>
                                            </View>
                                            {selectedPhotoUri ? (
                                                <View style={styles.selectedImageContainer}>
                                                    <Image style={styles.selectedImage} source={{ uri: selectedPhotoUri }} />
                                                </View>
                                            ) : null}
                                        </View>
                                    ) : (
