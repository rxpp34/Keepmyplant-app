...
{   
    ...
    function SendAbonnement() {
        ...
        axios({
            method: 'POST',
            url: "http://codx.fr:8080/SendAbonnementByBotaniste/" + route.params._iduser + "/" + route.params._idAnnonce
        }).then((response) => {
            if (response.data === "OK") {
                alert("RESERVATION OK")
            }
...
<ScrollView style={CSS.MainView}>
            ...
            {User.idRole === 1 ? 
                ...
                :
                <Pressable style={CSS.PressableButtonDemande} onPress={() => { SendAbonnement() }}>
                    <Text style={{ color: "#18c924", fontSize: 22, padding: 5, fontWeight: 'bold', textAlign: 'center' }}> S'abonner </Text>
                </Pressable>
